...
public class Polynom implements Polynom_able 
{
	...
	/**
	 * This is a constructor, that init a Polynom from a String such as: {"x",
	 * "3+1.4X^3-34x"} with no complex polynom
	 * 
	 * @param s:
	...
	/**
	 * This function calculate the y value after filling x in f(x) polynom. user the
	 * f function of the monom class
	 * 
	 * @param x:
	 *            value of x to be put in f(x)
	...
	/**
	 * This function substract(minus) polynom from existing polynom The substract
	 * function use the add function by sending the monom with minus one (-1) before
	 * 
	 * @param p1:
	...
	/**
	 * This function multiply existing polynom with a new polyonm. run over each
	 * monom in the new and the existing polynoms and calculate the new monom.
	 * insert the new calculated monon to a new polynom send the polynom to a remove
	 * function to union same power monoms(if those were created during multiply)
	 * 
	 * @param
	 */
	@Override
	...
	/**
	 * This function is a boolean function that compare two polynoms. Use
	 * comperator.
	 * 
	 * @param p1
	 *            represent the polynom that we want to compare with the existing
	 *            polynom.
	...
	public boolean equals(Object p1)
	{
		...
		while (iter1.hasNext() && iter2.hasNext())
		{
			if (!iter1.next().equals(iter2.next())
				...
				// double
				// temp1=Math.abs(iter1.next().get_coefficient()-iter2.next().get_coefficient());
				flag = false;
			}
		}
	 ...
	 * @param x1
	 *            represent a end point
	 * @param eps
	 *            represent the step (positive) value
	...
	/**
	 * This function create a deep copy of current Polynom. No parameters
	 * 
	 ...
	 *            end point
	 * @param eps>0
	 *            (positive) representing the epsilon range the solution should be
	 *            within.
	...
	/**
	 * This function get a polynom and in case there are duplicate powers, union the
	 * monoms. used in multiply function
	 * 
	 * @param tempPolynom
	 */
	private void RemoveSamePower(Polynom tempPolynom) 
	{
		...
		for (int i = 0; i < tempPolynom.listOfMonom.size();) 
		{
			...
			if (_PolynomComp.compare(tempPolynom.listOfMonom.get(i), tempPolynom.listOfMonom.get(i + 1)) == 0) {
				tempPolynom.listOfMonom.get(i).set_coefficient(tempPolynom.listOfMonom.get(i).get_coefficient()
	...
	/**
	 * This function calculate the string of the polynom move over all the monoms
	 * and concatenated them to string
	 * 
	 * @return new string
	 */
	@Override
